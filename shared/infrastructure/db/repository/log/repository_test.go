...
func (suite *TestSuite) TestSaveSuccess() {
	...
	suite.mock.
		ExpectExec(regexp.QuoteMeta("INSERT INTO `logs` (`created_at`,`description`,`detail`,`resource_id`) VALUES (?,?,?,?)")).
		WithArgs(test_utils.AnyTime{}, suite.description, suite.detail, suite.resourceId).
...
func (suite *TestSuite) TestSaveError() {
	...
	suite.mock.
		ExpectExec(regexp.QuoteMeta("INSERT INTO `logs` (`created_at`,`description`,`detail`,`resource_id`) VALUES (?,?,?,?)")).
		WithArgs(test_utils.AnyTime{}, suite.description, nil, suite.resourceId).
...
func (suite *TestSuite) TestSearchAllByExampleSuccess() {
	...
	suite.mock.
		ExpectQuery(regexp.QuoteMeta("SELECT * FROM `logs` WHERE `logs`.`resource_id` = ? ORDER BY created_at desc LIMIT 20")).
		WithArgs(suite.resourceId).
		WillReturnRows(sqlmock.
			NewRows([]string{"id", "created_at", "description", "detail", "resource_id"}).
			AddRow(999, time.Now(), suite.description, suite.detail, suite.resourceId).
			AddRow(1000, time.Now(), suite.description, nil, suite.resourceId),
		)
