...
func (suite *TestSuite) TestCreateSuccess() {
	...
	suite.mockUserRepo.On("Create", ctx, user).Return(uint(999), nil)

	res, err := suite.app.Create(ctx, suite.uid, suite.email)
...
func (suite *TestSuite) TestCreateErrorCreate() {
	...
	suite.mockUserRepo.On("ExistsByUID", ctx, suite.uid).Return(false, nil)
	suite.mockUserRepo.On("ExistsByEmail", ctx, suite.email).Return(false, nil)
	suite.mockUserRepo.On("Create", ctx, user).Return(uint(0), errExpected)
...
func (suite *TestSuite) TestLoginSuccess() {
	...
	suite.mockUserTokenRepo.On("Create", ctx, mock.Anything).Return(uint(0), nil)

	token, err := suite.app.Login(ctx, suite.uid, suite.email)
...
func (suite *TestSuite) TestLoginErrorCreateUserToken() {
	...
	suite.mockUserRepo.On("FindByUIDAndEmail", ctx, suite.uid, suite.email).Return(userExpected, nil)
	suite.mockUserTokenRepo.On("Create", ctx, mock.Anything).Return(uint(0), errExpected)
